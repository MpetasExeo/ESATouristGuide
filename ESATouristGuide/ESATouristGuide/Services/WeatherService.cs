    ...
    public class WeatherService : IWeatherService
    {
        ...
        public async Task<Temperatures> GetCurrentWeatherAsync(Location location , CancellationToken cancellationToken)
        {
            ...
            {             //Rootobject rawWeather = new Rootobject();
                ...
                var response = await _httpClient.GetAsync($"/data/2.5/weather?lat={location.Latitude.ToString(culture)}&lon={location.Longitude.ToString(culture)}&appid={"c7f584767245ebb4f4b36ec0426dda9f"}&units=metric").ConfigureAwait(true);
